...
// ─── LIAISON ENTRE LIGNES (multiplication automatique) ────────────────────────
// Quand on saisit une quantité "C" sur une référence source, la quantité
// (jour + poste identiques) est multipliée et injectée dans la référence cible,
// UNIQUEMENT si la cible n'a pas déjà une quantité saisie pour ce jour/poste.
...
const LIGNE_LINKS: LigneLink[] = [
  { ligneSource: 'L36:MXT5', refSource: '1SDN000070A1801', ligneCible: 'L40:GR TERMICO XT5', refCible: '1SDR002907A1801', multiplicateur: 3 },
  { ligneSource: 'L36:MXT5', refSource: '1SDN000070A1803', ligneCible: 'L40:GR TERMICO XT5', refCible: '1SDR002883A1801', multiplicateur: 3 },
  { ligneSource: 'L36:MXT5', refSource: '1SDN000070A1804', ligneCible: 'L40:GR TERMICO XT5', refCible: '1SDR002976A1801', multiplicateur: 3 },
  { ligneSource: 'L36:MXT5', refSource: '1SDN000223A1801', ligneCible: 'L40:GR TERMICO XT5', refCible: '1SDR002907A1802', multiplicateur: 3 },
];
...
function findLigneLink(ligne: string, reference: string): LigneLink | undefined {
  const nLigne = normalizeKey(ligne);
  const nRef = normalizeKey(reference);
  return LIGNE_LINKS.find(l =>
    normalizeKey(l.ligneSource) === nLigne && normalizeKey(l.refSource) === nRef
  );
}
  ...
  saveCell(ref: ReferenceRow, day: string): void {
    ...
    this.semaineService.updatePlanificationByCriteria(payload).subscribe({
      next: () => {
        ...
        // ✅ Liaison automatique vers la ligne/référence cible (si mapping existant)
        this.applyLigneLink(ref, day, newC);
      },
      error: () => { this.errorMessage = 'Erreur lors de la sauvegarde'; this.savingKey = null; }
    });
  }

  // ─── Liaison automatique entre lignes ────────────────────────────────────
  /**
   * Si `ref` (ligne + référence) fait partie de LIGNE_LINKS, calcule la quantité
   * cible (newC * multiplicateur) et la sauvegarde sur la référence cible,
   * pour le même jour et le même poste — mais UNIQUEMENT si la cellule cible
   * n'a pas déjà une quantité renseignée (> 0). Sinon on ne touche à rien.
   */
  private applyLigneLink(ref: ReferenceRow, day: string, newC: number): void {
    const link = findLigneLink(ref.ligne, ref.reference);

    console.log('[LigneLink] Vérification liaison pour', ref.ligne, '/', ref.reference, '→ link trouvé:', link);

    if (!link) return; // cette référence n'a pas de liaison configurée

    // On relit les planifications de la semaine pour connaître l'état actuel de la cible
    this.semaineService.getPlanificationsForWeek(this.selectedSemaine).subscribe({
      next: (planifResponse) => {
        const planifications: any[] = planifResponse?.planifications || [];
        const nLigneCible = normalizeKey(link.ligneCible);
        const nRefCible = normalizeKey(link.refCible);

        const existingCible = planifications.find(p =>
          normalizeKey(p.ligne) === nLigneCible &&
          normalizeKey(p.reference) === nRefCible &&
          normalizeKey(p.jour) === normalizeKey(day) &&
          normalizeKey(p.poste || 'poste1') === normalizeKey(this.selectedPoste)
        );

        console.log('[LigneLink] Cible existante trouvée pour', link.ligneCible, '/', link.refCible, '/', day, ':', existingCible);

        // ✅ Règle demandée : si la cible a déjà une quantité, on laisse telle quelle
        if (existingCible && existingCible.qtePlanifiee && existingCible.qtePlanifiee > 0) {
          console.log('[LigneLink] Cible déjà renseignée (', existingCible.qtePlanifiee, ') → on ne touche pas.');
          return;
        }

        const qteCalculee = newC * link.multiplicateur;

        const payloadCible = {
          ...this.semaineService.formatWeekForAPI({
            semaine: this.selectedSemaine,
            jour: day,
            ligne: link.ligneCible,
            reference: link.refCible,
            nbOperateurs: existingCible?.nbOperateurs || 0,
            of: existingCible?.of || '',
            qtePlanifiee: qteCalculee,
            qteModifiee: existingCible?.qteModifiee || 0,
            decProduction: existingCible?.decProduction || 0,
            decMagasin: existingCible?.decMagasin || 0
          }),
          poste: this.selectedPoste
        };

        console.log('[LigneLink] Envoi payload vers cible:', payloadCible);

        this.semaineService.updatePlanificationByCriteria(payloadCible).subscribe({
          next: (resp) => {
            console.log('[LigneLink] ✅ Succès mise à jour cible:', resp);
            this.showSuccess(`✓ ${link.refCible} mis à jour automatiquement (${qteCalculee})`);
            this.checkMissingOfForLine(link.ligneCible);
            this.checkMissingOfForAllLines();

            // Si la ligne cible est actuellement affichée à l'écran, on rafraîchit le tableau
            if (this.selectedLigneForView?.ligne === link.ligneCible) {
              this.loadDataForLigne(link.ligneCible);
            }
          },
          error: (err) => {
            console.error('[LigneLink] ❌ Erreur lors de la mise à jour automatique de la référence liée', link.refCible, err);
          }
        });
      },
      error: (err) => {
        console.error('[LigneLink] ❌ Erreur lors de la récupération des planifications pour la liaison inter-lignes', err);
      }
    });
  }
