...
import { AffectationService, Affectation } from '../affectation/affectation.service';
...
showMPSuggestions = signal(false);
  ...
  // ✅ NOUVEAU — Présents / Absents / En congé pour la ligne sélectionnée
  lignePresents = signal<number>(0);
  ligneAbsents = signal<number>(0);
  ligneConges = signal<number>(0);
  ligneTotal = signal<number>(0);
  loadingPresence = signal<boolean>(false);

  lignePresentsListe = signal<Affectation[]>([]);
  ligneAbsentsListe = signal<Affectation[]>([]);
  ligneCongesListe = signal<(Affectation & { statutConge?: string; dateDebut?: string; dateFin?: string; commentaire?: string })[]>([]);

  presenceListModalType = signal<'presents' | 'absents' | 'conges' | null>(null);
...
onLigneSelected(line: ProductionLine): void {
  ...
  // ✅ NOUVEAU — Charger présents/absents/congés pour cette ligne
...
private loadPresenceForLigne(ligne: string): void {
  ...
  this.ligneConges.set(0);
  this.ligneTotal.set(0);
  this.lignePresentsListe.set([]);
  this.ligneAbsentsListe.set([]);
  this.ligneCongesListe.set([]);

  // Appels parallèles : affectations + pointage du jour (présents/absents/congés)
  forkJoin({
    ...
    next: ({ affectations, pointage }) => {
      // ⚠️ pointage typé sans enConge côté affectation.service.ts (interface à mettre à jour) → cast temporaire
      const pointageData: any = pointage;

      // Ouvriers affectés à cette ligne
      const ouvriersDeLaLigne = affectations.data.filter(a => a.ligne === ligne);

      // Matricules présents aujourd'hui (badge ou statut manuel "présent")
      const matriculesPresents = new Set(
        (pointageData.presents || []).map((p: any) => Number(p.matricule))
      );

      // ✅ Matricules en congé/maladie/mission/etc. → Map pour récupérer dateDebut/dateFin
      const congesParMatricule = new Map<number, any>();
      (pointageData.enConge || []).forEach((c: any) => {
        congesParMatricule.set(Number(c.matricule), c);
      });

      const presentsListe: Affectation[] = [];
      const absentsListe: Affectation[] = [];
      const congesListe: any[] = [];

      ouvriersDeLaLigne.forEach(ouvrier => {
        const matricule = Number(ouvrier.matricule);
        if (matriculesPresents.has(matricule)) {
          presentsListe.push(ouvrier);
        } else if (congesParMatricule.has(matricule)) {
          const conge = congesParMatricule.get(matricule);
          congesListe.push({
            ...ouvrier,
          ...
          absentsListe.push(ouvrier);
        }
      });

      this.ligneTotal.set(ouvriersDeLaLigne.length);
      this.lignePresents.set(presentsListe.length);
      this.ligneAbsents.set(absentsListe.length);
      this.ligneConges.set(congesListe.length);
      this.lignePresentsListe.set(presentsListe);
      this.ligneAbsentsListe.set(absentsListe);
      this.ligneCongesListe.set(congesListe);
...
// ✅ NOUVEAU — Libellé lisible pour un statut de congé/absence justifiée
statutCongeLabel(statut?: string): string {
  ...
  return statut ? (labels[statut] || statut) : 'Justifié';
}

// ✅ NOUVEAU — ouvrir/fermer la modale de consultation des listes
openPresenceListModal(type: 'presents' | 'absents' | 'conges'): void {
  this.presenceListModalType.set(type);
}

closePresenceListModal(): void {
  this.presenceListModalType.set(null);
}
