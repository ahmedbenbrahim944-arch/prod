...
// ✅ NOUVEAU — pour "Écart principal" dans Récup Production (pointage-dashboard)
export interface Causes7MLigne {
...
export interface StatLigne7M {
  ligne: string;
  causes7M: Causes7MLigne;
}

export interface StatsPeriodeLight {
  statsParLigne: StatLigne7M[];
}
...
export class StatsService1 {
  ...
  getStatsPeriode(dateDebut: string, dateFin: string, poste?: string): Observable<StatsPeriodeLight> {
  const params: any = { dateDebut, dateFin };
  if (poste) params.poste = poste;
  return this.http.get<StatsPeriodeLight>(`${this.apiUrl}/stats-periode`, {
    params,
    headers: this.getHeaders()
  });
}
