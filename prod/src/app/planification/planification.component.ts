...
// ✅ NOUVEAU — listes détaillées + modal de consultation
lignePresentsListe = signal<Affectation[]>([]);
ligneAbsentsListe = signal<Affectation[]>([]);
presenceListModalType = signal<'presents' | 'absents' | null>(null);
  ...
  forkJoin({
    ...
    next: ({ affectations, pointage }) => {
      ...
      ouvriersDeLaLigne.forEach(ouvrier => {
        if (matriculesPresents.has(Number(ouvrier.matricule))) {
...
openPresenceListModal(type: 'presents' | 'absents'): void {
