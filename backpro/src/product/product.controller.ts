...
async create(
  @Body() body: any,   // on reçoit en any pour pouvoir parser avant validation
  @UploadedFile() imageFile: any,
  @Req() req
) {
  // Parser references si elle arrive en string JSON (cas FormData/multipart)
  if (typeof body.references === 'string') {
    try {
      body.references = JSON.parse(body.references);
    } catch (e) {
      throw new BadRequestException('Format de références invalide');
    }
  }

  // Valider manuellement maintenant que le format est correct
  const createProductDto = plainToInstance(CreateProductDto, body);
  const errors = await validate(createProductDto, { whitelist: true, forbidNonWhitelisted: true });
  if (errors.length > 0) {
